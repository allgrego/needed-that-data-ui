...
import type { MonitorHistoryRatesData } from "@/modules/exchangeRates/types/monitor-dolar.types";
...
function mean(arr: number[], n: number) {
  ...
  for (let i = 0; i < n; i++) sum = sum + arr[i];
  return sum / n;
}

const MonitorDolarLineChart: FC<MonitorDolarLineChartProps> = ({
  dataQuery,
}) => {
  try {
    // When error
    if (dataQuery.isError)
      return <EmptyMessage>Chart unavailable⛔</EmptyMessage>;
    // First or retry loading
    if (
      (!dataQuery.isFetched && dataQuery.isFetching) ||
      (dataQuery.isFetched && !dataQuery.data && dataQuery.isFetching)
    ) {
      return (
        <EmptyMessage>
          <Spinner />
    ...
    const labels = (dataQuery.data?.rates || [])
      .map((rate) => {
    ...
    const dataRates = (dataQuery.data?.rates || [])
      .map((rate) => Number(rate.usd))
      .reverse();
