...
import BcvRatesSkeleton from "@/modules/exchangeRates/components/bcvRatesSkeleton/BcvRatesSkeleton";
...
const MonitorDolarRates: FC<MonitorDolarRatesProps> = ({
  ratesHistoryData: data,
  isError = false,
  isFetching = false,
  isFetched = false,
  dataUpdatedAt = 0,
  ...
  const ratesHistory = data.rates.map((rate) => {
    ...
    // Set Timezome offset
    const offsetHours = process.env.NEXT_PUBLIC_MONITOR_HOURS_OFFSET
      ? Number(process.env.NEXT_PUBLIC_MONITOR_HOURS_OFFSET)
      : 0;
  ...
  const differencePercentage =
    ((Number(lastRate.usd) - Number(beforeLastRate.usd)) /
      Number(beforeLastRate.usd)) *
    100;
  ...
  return (
    <>
        ...
        <table className="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            <tr className="bg-slate-100 border-b hover:bg-gray-50">
              <td
                scope="row"
                className="py-4 px-6 text-gray-800 font-medium whitespace-nowrap"
              >
                <div className="h-8 w-8 inline-block text-lime-800">
                  <CurrencyDollarIcon />
                </div>{" "}
                USD
              </td>
              <td className="py-4 px-6" colSpan={1}>
                <div className="inline-block font-bold text-gray-900 text-lg">
                  {lastRate.usd}
                </div>{" "}
              ...
              <td className="py-4" colSpan={1}>
                {
                  // TrendingUp
                  isTrendingUp ? (
                    <div className="inline-block font-bold text-red-700 text-md">
                      <ArrowTrendingUpIcon className="h-5 w-5 inline" />{" "}
                      <span>{differencePercentage.toFixed(2)}%</span>
                    </div>
                  ) : // Stable
                  isEqual ? (
                    <div className="inline-block font-bold text-blue-900 text-md">
                      <Bars2Icon className="h-4 w-4 inline" />
                    </div>
                  ) : (
                    // Trending Down
                    <div className="inline-block font-bold text-green-800 text-md">
                      <ArrowTrendingDownIcon className="h-5 w-5 inline" />{" "}
                      <span>{differencePercentage.toFixed(2)}%</span>
                    </div>
                  )
                }
      ...
      <div className="text-right text-sm mt-5 font-medium text-gray-500 mb-2">
        Published at: {lastRateDate.toLocaleString("es-US")}
      </div>
      <div
        className={`text-right text-xs mt-2 font-base text-gray-500 mb-10 ${
          isFetching && "animate-pulse"
        }`}
      >
        Updated at:{" "}
        {dataUpdatedAt ? new Date(dataUpdatedAt).toLocaleString("en-GB") : "-"}
      </div>
