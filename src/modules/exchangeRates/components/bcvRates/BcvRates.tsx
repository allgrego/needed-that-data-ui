...
const BcvRates: FC<BcvRatesProps> = ({
  ratesData: data,
  isError = false,
  isFetching = false,
  isFetched = false,
  ...
  // Array of rates info
  const rates: Record<string, any>[] = Object.entries(data.rates).map(
    ([code, value]) => {
      return {
        code,
  ...
  return (
    <>
            ...
            {rates.map(({ rate, code, name }) => (
              <tr key={code} className="bg-slate-100 border-b hover:bg-gray-50">
                <td className="py-4 px-6 text-gray-800 font-medium whitespace-nowrap">
                  <div className="h-8 w-8 inline-block text-indigo-800">
                    {currenciesIcons[code]}
                  </div>{" "}
                ...
                <td className="py-4 px-6">
                  ...
                  <span className="text-xs scale-50">
      ...
      <div className="text-right text-sm mt-5 font-medium text-gray-500 mb-2">
        Published at:{" "}
        {data.bcvDate
          ? new Date(data.bcvDate + "T08:00:00.000Z").toLocaleDateString(
              "en-GB"
            )
          : "-"}
      </div>
